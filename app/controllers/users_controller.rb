class UsersController < ApplicationController
  ...
  def follower_list
    @users = @user.followers.select(:name, :image).order(:id).page(params[:page]).per(6)
  end

  def following_list
    @users = @user.following.select(:name, :image).order(:id).page(params[:page]).per(6)
  end

  def search_result
   @users = User.search(params[:search]).select(:name, :image).page(params[:page]).per(6)
 end
